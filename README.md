<h1>
    <a href="https://www.laboratoria.la/br">
        <span>📘 Manual — Plataforma Pessoal</span>
    </a>
</h1>

Este manual serve como referência rápida para resolver problemas de Git
no repositório **personal-platform**, especialmente relacionados a
submódulos acidentais, estrutura de projetos e fluxo de commits.

## 🎯 Objetivo do Repositório

- Portfólio pessoal
- Plataforma central de projetos
- Next.js + TypeScript
- Testes unitários e coverage
- Estrutura com múltiplos subdomínios

## 🚀 Projetos em Produção

<table>
  <thead>
    <tr align="left">
      <th>Nº</th>
      <th>Descrição do Projeto</th>
      <th>Repositório do Projeto</th>
      <th>Link do Deploy</th>
    </tr>
  </thead>
  <tbody align="left">
    <tr>
      <td>01</td>
      <td>Descrição</td>
      <td align="center">
        <a href="https://burgersqueen.vercel.app/login" target="_blank">
           <img align="center" alt="Botao" src="https://img.shields.io/badge/Ver%20Github-1F1F1F?style=for-the-badge" width="120px">
        </a>
      </td>
      <td align="center">
        <a href="https://burgersqueen.vercel.app/login" target="_blank">
           <img align="center" alt="Botao" src="https://img.shields.io/badge/Ver%20Deploy-EA580C?style=for-the-badge" width="120px">
        </a>
      </td>
    </tr> 
  </tbody> 
</table>                                                                         

<div align="center">

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif"><br>

##### Desenvolvido por <span>Elizabete Fabri</span> 🩷

</div>
